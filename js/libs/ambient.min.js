...
var Ambient = (function () {
  ...
  Ambient.prototype.mount = function (element) {
    ...
    var zoomistImages = document.querySelectorAll("div[data-ambient-zoomist]");

    for (var i = 0; i < zoomistImages.length; i++) {
      var zoomistImage =
        zoomistImages[i].childNodes[1].childNodes[1].childNodes[1];
      // console.log(zoomistImages[i], zoomistImage);
      this.elements.push(renderImageZoomist(zoomistImages[i], zoomistImage));
    }
  ...
  function renderImageZoomist(zoomistElement, imgElement) {
    var container = document.createElement("div");
    container.className = "ambient__container";
    if (zoomistElement.getAttribute("data-ambient-zoomist").length > 0)
      container.id = zoomistElement.getAttribute("data-ambient-zoomist");
    var visibleElement;
    var originalAttributes = [];
    if (_retainAttributes) visibleElement = zoomistElement.cloneNode(true);
    else {
    ...
    shadowElement.src = imgElement.src;
    if (_customBlur) applyBlur(shadowElement, _customBlur);
    visibleElement.removeAttribute("data-ambient");
    ...
    zoomistElement.parentNode.insertBefore(container, zoomistElement);
    zoomistElement.parentNode.removeChild(zoomistElement);
    return new AmbientElement(container, originalAttributes);
  }
